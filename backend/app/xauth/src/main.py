...
logger = get_logger("xauth.plugin")
...
class Plugin(IPCCommands, AutoDispatchMixin, TrustedBase):
    ...
    async def on_load(self) -> None:
        ...
        try:
            ...
            logger.warning("auth : ext.email indisponible (%s) — envoi d'email désactivé.", exc)
            email_ext = None
        ...
        try:
            ...
            self._token_cipher = TokenCipher(env.get("OAUTH_TOKEN_KEY"))
        except Exception as exc:
            logger.warning(
                "auth : chiffrement des jetons OAuth indisponible (%s) — "
                "les jetons API tiers (Gmail/Calendar...) ne seront pas persistés.",
                exc,
            )
...
def _build_oauth_providers(env: dict, base_url: str = "http://localhost:8000") -> dict[str, OAuthProvider]:
    ...
    for name, cls, id_key, secret_key in _registry:
        ...
        if client_id and client_secret:
            # Le plugin est monté à /app/auth (app.plugin_prefix dans
            # integration.yaml, voir les logs de boot : "plugin routes
            # mounted plugin=auth ... prefix=/app/auth") — l'URI enregistrée
            # côté Google Cloud Console doit correspondre exactement à ce
            # que le serveur sert réellement, sans quoi Google refuse même
            # de rediriger vers nous (redirect_uri_mismatch).
