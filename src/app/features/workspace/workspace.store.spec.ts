...
import { provideTestTranslations } from '../../../testing/setup';
...
describe('WorkspaceStore — mémoire des dossiers', () => {
  let store: WorkspaceStore;
  ...
  it('donne à chaque dossier sa propre mémoire, sans recouvrement', () => {
    const [meridian, halloway] = store.projects();

    expect(meridian.memory.length).toBeGreaterThan(0);
    expect(halloway.memory.length).toBeGreaterThan(0);

    const ids = new Set(meridian.memory.map((n) => n.id));
    for (const note of halloway.memory) {
      expect(ids.has(note.id), 'une note ne peut pas appartenir à deux dossiers').toBe(false);
    }
  });

  it('range ce qu’elle retient chez le client ouvert, et nulle part ailleurs', () => {
    const [meridian, halloway] = store.projects();
    const before = halloway.memory.length;

    store.selectProject(meridian.id);
    store.remember({ text: 'Leur conseil répond sous 48 h.' });

    const after = store.projects();
    expect(after[0].memory[0].text).toBe('Leur conseil répond sous 48 h.');
    // Le dossier voisin n'a pas bougé — c'est tout l'enjeu de la séparation.
    expect(after[1].memory).toHaveLength(before);
    expect(store.companyMemory()).toHaveLength(0);
  });

  it('retient en mémoire commune hors de tout dossier', () => {
    store.remember({ text: 'Le comité financier siège le premier mardi.' });

    expect(store.companyMemory()).toHaveLength(1);
    for (const project of store.projects()) {
      expect(project.memory.some((n) => n.text?.includes('comité financier'))).toBe(false);
    }
  });

  it('ouvre un nouveau dossier sans mémoire', () => {
    const project = store.createProject('Ardent — audit', 'Ardent SA', 12);
    expect(project?.memory).toHaveLength(0);
  });
});
