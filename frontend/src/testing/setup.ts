...
/** N fichiers factices — pour les tests qui déposent des documents sans vraiment les lire. */
export function mockFiles(count: number): File[] {
  return Array.from({ length: count }, (_, i) => new File(['x'], `doc-${i}.pdf`));
}
